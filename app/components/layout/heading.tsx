...
export function Heading({ menus }: Readonly<{ menus: Menu[] }>): JSX.Element {
  return (
    <section className="px-3 space-y-6">
      ...
      <h1
        className={`fade-in-up font-bold font-maru text-4xl text-center ${menus.length === 1 ? menus[0].textColor : menus.at(-1)?.textColor}`}
      >
        {menus.at(-1)?.name}
      </h1>
    </section>
    ...
    >
      <ul className="flex items-center whitespace-nowrap">
        <li className="flex items-center">
          <Link
            href="/"
            className="button-pop flex items-center gap-1 p-1 rounded-xl underline hover:bg-gray-200"
          >
        ...
        {menus.map((menu, index) => (
          <li key={menu.name} className={`flex items-center ${menu.textColor}`}>
            {menus.length > 1 && index === 0 ? (
              <Link
                href={menu.pathname}
                className="button-pop p-1 rounded-xl underline hover:bg-gray-200"
              >
                {menu.name}
              </Link>
            ) : (
