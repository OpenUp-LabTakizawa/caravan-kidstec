...
export default function History(): JSX.Element {
  return (
    <>
      ...
      <h2 className="font-bold font-zenMaruGothic text-3xl">{KANTO.name}</h2>
      {KANTO_HISTORY.map((history) => (
        <section key={history.date} className="space-y-4">
          <details className="collapse collapse-arrow">
            ...
            <PictureTile
              pathname={KANTO.pathname}
              date={history.date}
              pictures={history.pictures}
            />
            <Link
              href={`${HISTORY.pathname}/movie${KANTO.pathname}/${history.date}`}
